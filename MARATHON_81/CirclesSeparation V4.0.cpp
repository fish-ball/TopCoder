...
inline bool overlap(const Circle& a, const Circle& b) {
    return dist(a.c, b.c) < a.r+b.r-EPS;
    //(a.r+b.r)*(a.r+b.r) - a.c.x-b.c.x)*(a.c.x-b.c.x)+(a.c.y-b.c.y)*(a.c.y-b.c.y) > EPS;
}
...
class CirclesSeparation {
    ...
    bool __isValid(const Circle& c, int k) {
        for(int i = 0; i < k; ++i) {
            if(overlap(C[i], c)) return false;
        }
        return true;
    }
    /*
