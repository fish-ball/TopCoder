...
public:
    ...
    Segment getSegment(const Line& line
        ...
        if(x2 < x1) x2 = W;
        if(y2 < y1) y2 = H;
        if(tolerance == -1) tolerance = H / 200;
    ...
    void drawLine(const Line& line, int rr=0xFF, int gg=0, int bb=0) {
        ...
        // 找到边界交点 
        p = inside(p) ? p : intersect(line, Line(1, 0, 1));
        p = inside(p) ? p : intersect(line, Line(0, 1, 1));
        p = inside(p) ? p : intersect(line, Line(1, 0, 1-W));
        p = inside(p) ? p : intersect(line, Line(0, 1, 1-H));
...
public:
    ...
    vector <string> recognizeObjects(
        ...
        // 根据噪声情况决定是否滤波 
        double diff = 0;
        Image imgA(limg), imgB(rimg);
        
        for(int i = 1; i < imgA.W; ++i) {
            for(int j = 1; j < imgA.H; ++j) {
                int rr = 0 + imgA.r(i,j) + imgA.r(i,j) - imgA.r(i,j-1) - imgA.r(i-1,j);
                int gg = 0 + imgA.g(i,j) + imgA.g(i,j) - imgA.g(i,j-1) - imgA.g(i-1,j);
                int bb = 0 + imgA.b(i,j) + imgA.b(i,j) - imgA.b(i,j-1) - imgA.b(i-1,j);
...
//*
        Image imgAf = diff < 6 ? Image(1,1) : imgA.getFilted();
        
        Image& img1 = diff < 6 ? imgA : imgAf;
        v = img1.getVertex();
        for(int i = 0; i < 22; ++i) {
            Point p = getPoint(PS[i][0], PS[i][1]);
            X1[i] = int(p.x+0.5);
            Y1[i] = int(p.y+0.5);
            if(!img1.inside(X1[i], Y1[i])) {
                X1[i] = W/2;
                Y1[i] = H/2;
            }
        }
...
//*/
//*
        Image imgBf = diff < 6 ? Image(1,1) : imgB.getFilted();
        
        Image& img2 = diff < 6 ? imgB : imgBf;
        v = img2.getVertex();
        for(int i = 0; i < 22; ++i) {
            Point p = getPoint(PS[i][0], PS[i][1]);
            X2[i] = int(p.x+0.5);
            Y2[i] = int(p.y+0.5);
            if(!img2.inside(X2[i], Y2[i])) {
                X2[i] = W/2;
                Y2[i] = H/2;
            }
        }
